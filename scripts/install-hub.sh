...
Options (anything you leave out is asked interactively):
  ...
  --ssl domain|files|none  certificate mode
...
if [ -z "$SSL_MODE" ] && [ "$INTERACTIVE" = "1" ]; then
  ...
  echo -e "  ${green}2.${plain} HTTPS with certificate files you already have"
  echo -e "  ${green}3.${plain} Plain HTTP — only safe behind a reverse proxy or on a private network"
  echo -e "${blue}Note:${plain} option 1 needs a domain already pointed at this server and port ${ACME_HTTP_PORT} reachable."
  choice=""
...
case "$SSL_MODE" in
  ...
  files)
    ...
    ;;
  none) ;;
  *) fail "--ssl must be one of: domain, files, none" ;;
esac
...
if [ "$SSL_MODE" = "domain" ]; then
  ...
  # --installcert also registers the renewal hook, so the daily acme.sh cron
  # copies the renewed pair here and restarts the hub without anyone watching.
  # Its exit code is unusable here: the reloadcmd runs immediately and the unit
  # does not exist yet, so the files are what we check.
